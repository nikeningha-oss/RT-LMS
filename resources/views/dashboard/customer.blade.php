...
@section('title', 'Customer Dashboard - Tracklane')
...
<div style="background:#F8FAFC; padding:18px 22px; min-height:560px;">
    ...
    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:16px;">
        <div>
            <div style="font-size:16px; font-weight:500; color:#0F172A;">Hi {{ Auth::user()->name }} 👋</div>
    ...
    <!-- ============================================================
         ...
         ============================================================ -->
    @if(isset($pendingPriceOrders) && $pendingPriceOrders->count() > 0)
        @foreach($pendingPriceOrders as $order)
        <div class="price-notification">
            <div>
                ...
                <div style="margin-top:4px;">
                    <span class="order-ref">Order #{{ $order->order_number }}</span>
                    <span style="margin:0 8px; color:#94A3B8;">|</span>
                    <span style="font-size:13px; color:#0F172A;">
                        Delivery: <strong>{{ Str::limit($order->delivery_address, 30) }}</strong>
                    </span>
                </div>
                <div style="margin-top:4px; display:flex; align-items:center; gap:12px; flex-wrap:wrap;">
                    ...
                    <span class="price-amount">{{ number_format($order->total_price, 0, ',', ' ') }} F</span>
                    <span style="font-size:12px; color:#94A3B8;">(Set by admin)</span>
                </div>
            </div>
            <div style="display:flex; gap:8px; flex-wrap:wrap;">
                <button onclick="confirmPrice({{ $order->id }})" class="btn-confirm">
                    <i class="ti ti-check"></i> Confirm Price
                </button>
                <button onclick="window.location.href='{{ route('customer.orders') }}'" 
    ...
    <!-- ============================================================
         ...
         ============================================================ -->
    @if(isset($confirmedOrders) && $confirmedOrders->count() > 0)
        ...
        <div style="background:#D1FAE5; border:1px solid #10B981; border-radius:12px; padding:16px 18px; margin-bottom:16px; display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px;">
            <div>
                ...
                <div style="margin-top:4px;">
                    ...
                    <span style="margin:0 8px; color:#94A3B8;">|</span>
                    <span style="font-size:13px; color:#0F172A;">
                        Total: <strong>{{ number_format($order->total_price, 0, ',', ' ') }} F</strong>
                    </span>
                </div>
            </div>
            <div style="display:flex; gap:8px; flex-wrap:wrap;">
                <button onclick="makePayment({{ $order->id }})" class="btn-pay">
                    <i class="ti ti-credit-card"></i> Pay Now
                </button>
                <button onclick="window.location.href='{{ route('tracking', $order->id) }}'" 
    ...
    <div style="display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:10px; margin-bottom:16px;">
        <div style="background:#FFFFFF; border:0.5px solid #E2E8F0; border-radius:8px; padding:11px 12px;">
            ...
            <div style="font-size:18px; font-weight:500; color:#0F172A;">{{ $totalOrders ?? 0 }}</div>
        </div>
        <div style="background:#FFFFFF; border:0.5px solid #E2E8F0; border-radius:8px; padding:11px 12px;">
            <div style="font-size:12px; color:#64748B; margin-bottom:6px;">This month</div>
            <div style="font-size:18px; font-weight:500; color:#0F172A;">{{ $monthlyOrders ?? 0 }}</div>
        </div>
        <div style="background:#FFFFFF; border:0.5px solid #E2E8F0; border-radius:8px; padding:11px 12px;">
            ...
            <div style="font-size:18px; font-weight:500; color:#F59E0B;">
                {{ isset($pendingPaymentCount) ? $pendingPaymentCount : 0 }}
            </div>
        </div>
    </div>
            ...
            <table style="width:100%; font-size:12px; border-collapse:collapse;">
                ...
                <tbody>
                    ...
                    <tr style="border-top:0.5px solid #E2E8F0;">
                        <td style="padding:7px 4px; color:#0F172A; font-weight:500;">{{ $order->order_number }}</td>
                        <td style="padding:7px 4px; color:#0F172A;">{{ Str::limit($order->delivery_address, 20) }}</td>
                        <td style="padding:7px 4px; color:#64748B;">
                            {{ optional($order->created_at)->format('M d') ?? 'N/A' }}
                        </td>
                        <td style="padding:7px 4px;">
                            ...
                            @endphp
                            <span style="padding:3px 8px; border-radius:6px; {{ $statusStyles[$order->status] ?? 'background:#F1F5F9;color:#475569;' }}">
