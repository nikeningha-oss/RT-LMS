...
@section('title', 'Settings - Tracklane')
...
<div style="background:#F8FAFC; padding:18px 22px; min-height:560px;">
    ...
    <div style="max-width: 900px; margin: 0 auto;">
            ...
            <form method="POST" action="{{ route('settings.update-profile') }}">
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:12px;">
                    <div>
                        ...
                        <input type="text" name="name" class="form-control-sm-custom" value="{{ Auth::user()->name }}" required>
                    </div>
                    <div>
                        <label style="font-size:12px; font-weight:500; color:#0F172A; display:block; margin-bottom:4px;">Email Address</label>
                        <input type="email" name="email" class="form-control-sm-custom" value="{{ Auth::user()->email }}" required>
                    </div>
                </div>
        ...
        <div class="settings-card" style="border-left: 3px solid #14B8A6;">
            ...
            <form method="POST" action="{{ route('settings.update-driver') }}">
                @csrf
                @method('PUT')
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr 1fr; gap:12px;">
                    <div>
                        ...
                        <input type="text" name="phone" class="form-control-sm-custom" value="{{ Auth::user()->driver->phone ?? '' }}" placeholder="6 77 12 34 56">
                    </div>
                    <div>
                        <label style="font-size:12px; font-weight:500; color:#0F172A; display:block; margin-bottom:4px;">Vehicle Model</label>
                        <input type="text" name="vehicle_model" class="form-control-sm-custom" value="{{ Auth::user()->driver->vehicle->model ?? '' }}" placeholder="Honda CB125">
                    </div>
                    <div>
                        <label style="font-size:12px; font-weight:500; color:#0F172A; display:block; margin-bottom:4px;">Plate Number</label>
                        <input type="text" name="vehicle_plate" class="form-control-sm-custom" value="{{ Auth::user()->driver->vehicle->plate_number ?? '' }}" placeholder="KJ 412 X">
                    </div>
                </div>

                <div style="display:flex; align-items:center; gap:12px; margin-top:12px;">
                    <label class="toggle-switch">
                        <input type="checkbox" name="is_available" {{ Auth::user()->driver->is_available ? 'checked' : '' }}>
                        <span class="toggle-slider-btn"></span>
                    </label>
        ...
        <div class="settings-card" style="border-left: 3px solid #E24B4A;">
            ...
            <form method="POST" action="{{ route('settings.update-admin') }}">
                @csrf
                @method('PUT')
