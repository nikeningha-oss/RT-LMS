...
<div class="topbar">
    <div class="brand">
        ...
        <?php endif; ?>
        <a href="<?php echo e(route('dashboard')); ?>" class="back-btn" style="margin-left:12px;">
            <i class="ti ti-arrow-left" style="font-size:16px;"></i> Dashboard
        </a>
    </div>
    <div class="topbar-right">
        <span class="role-badge <?php echo e(Auth::user()->role); ?>">
            <?php if(Auth::user()->role === 'customer'): ?> 👤 Customer
            <?php elseif(Auth::user()->role === 'driver'): ?> 🚚 Driver
            <?php else: ?> 🛡️ Admin
            <?php endif; ?>
        </span>
    </div>
    ...
    <div class="panel" id="panel">
        <?php if(!isset($order) || !$order): ?>
            <div class="no-order">
                ...
                <a href="<?php echo e(route('dashboard')); ?>" class="btn btn-primary" style="margin-top:8px; display:inline-flex;">
...
<script>
    (function() {
        ...
        var trackingData = <?php echo json_encode($trackingData ?? null, 15, 512) ?>;
        var allOrdersData = <?php echo json_encode($allOrders ?? [], 15, 512) ?>;
        var userRole = <?php echo json_encode(Auth::user()->role, 15, 512) ?>;
        var orderId = <?php echo json_encode(isset($order) ? $order->id : null, 15, 512) ?>;
        var orderNumber = <?php echo json_encode(isset($order) ? $order->order_number : null, 15, 512) ?>;
        var driverId = <?php echo json_encode(isset($order) && $order->driver ? $order->driver->id : null, 15, 512) ?>;
        var dashboardUrl = <?php echo json_encode(route('dashboard'), 15, 512) ?>;
        var maptilerKey = <?php echo json_encode(env('MAPTILER_KEY'), 15, 512) ?>;
...
function calculateDistance(lat1, lon1, lat2, lon2) {
    ...
    return R * c;
        ...
        function renderDriverPanel(data) {
            ...
            var cName = '<?php echo e(isset($order) && $order->customer ? addslashes($order->customer->name) : "Customer"); ?>';
            var cPhone = '<?php echo e(isset($order) && $order->customer ? addslashes($order->customer->phone ?? "N/A") : "N/A"); ?>';
            html += '<div class="card">';
        ...
        function renderAdminPanel(data) {
            ...
            if (!data.driver && data.status !== 'delivered' && data.status !== 'cancelled') {
                html += '<a href="<?php echo e(route("admin.orders.assign", isset($order) ? $order->id : 0)); ?>" class="btn btn-primary" style="width:100%;"><i class="ti ti-user-plus"></i> Assign Driver</a>';
            }
...
</html><?php /**PATH C:\xampp\htdocs\logistics_system\resources\views/tracking/index.blade.php ENDPATH**/ ?>