...
<?php $__env->startSection('title', 'Manage Orders - Tracklane'); ?>
...
<div style="background:#F8FAFC; padding:18px 22px; min-height:560px;">
    ...
    <div style="display:grid; grid-template-columns:repeat(6,minmax(0,1fr)); gap:8px; margin-bottom:16px;">
        <div style="background:#FFFFFF; border:0.5px solid #E2E8F0; border-radius:8px; padding:8px 12px; text-align:center;">
            ...
            <div style="font-size:16px; font-weight:600; color:#0F172A;"><?php echo e($orders->total() ?? 0); ?></div>
        </div>
        <div style="background:#FFFFFF; border:0.5px solid #E2E8F0; border-radius:8px; padding:8px 12px; text-align:center;">
            <div style="font-size:10px; color:#64748B;">Pending</div>
            <div style="font-size:16px; font-weight:600; color:#F59E0B;">
                <?php echo e($orders->where('status', 'pending')->count() ?? 0); ?>
        ...
        <div style="background:#FFFFFF; border:0.5px solid #E2E8F0; border-radius:8px; padding:8px 12px; text-align:center;">
            ...
            <div style="font-size:16px; font-weight:600; color:#F59E0B;">
                <?php echo e($orders->where('status', 'price_pending')->count() ?? 0); ?>
        ...
        <div style="background:#FFFFFF; border:0.5px solid #E2E8F0; border-radius:8px; padding:8px 12px; text-align:center;">
            ...
            <div style="font-size:16px; font-weight:600; color:#3B82F6;">
                <?php echo e($orders->where('status', 'in_transit')->count() ?? 0); ?>
        ...
        <div style="background:#FFFFFF; border:0.5px solid #E2E8F0; border-radius:8px; padding:8px 12px; text-align:center;">
            ...
            <div style="font-size:16px; font-weight:600; color:#10B981;">
                <?php echo e($orders->where('status', 'delivered')->count() ?? 0); ?>
        ...
        <div style="background:#FFFFFF; border:0.5px solid #E2E8F0; border-radius:8px; padding:8px 12px; text-align:center;">
            ...
            <div style="font-size:16px; font-weight:600; color:#E24B4A;">
                <?php echo e($orders->where('status', 'cancelled')->count() ?? 0); ?>
                ...
                <tbody>
                    ...
                    <tr style="border-bottom:0.5px solid #E2E8F0;">
                        <td style="padding:8px; font-weight:500; color:#0F172A;">
                            ...
                            <br>
                            <span class="price-breakdown-toggle" onclick="toggleBreakdown(<?php echo e($order->id); ?>)">
                                <i class="ti ti-chevron-down" id="toggleIcon<?php echo e($order->id); ?>"></i> Breakdown
                            </span>
                        </td>
                        ...
                        <td style="padding:8px; color:#64748B;">
                            <?php echo e($order->driver->name ?? 'Unassigned'); ?>

                        </td>
                        <td style="padding:8px; color:#64748B;">
                            <?php echo e($order->distance_km ?? 0); ?> km
                        </td>
                        <td style="padding:8px; color:#64748B;">
                            <?php echo e($order->weight_kg ?? 0); ?> kg
                        </td>
                        <td style="padding:8px;">
                            <span class="status-<?php echo e($order->status); ?>" style="padding:3px 10px; border-radius:12px; font-size:11px;">
                        ...
                        <td style="padding:8px; text-align:right; font-weight:500; color:#10B981;">
                            <?php if($order->driver_earning > 0): ?>
                                <?php echo e(number_format($order->driver_earning, 0, ',', ' ')); ?> F
                            <?php else: ?>
                        ...
                        <td style="padding:8px; text-align:right; font-weight:500; color:#3B82F6;">
                            <?php if($order->platform_fee > 0): ?>
                                <?php echo e(number_format($order->platform_fee, 0, ',', ' ')); ?> F
                            <?php else: ?>
                                <span style="color:#94A3B8;">—</span>
                            <?php endif; ?>
                        </td>
                        <td style="padding:8px; text-align:center;">
                            ...
                            <?php elseif($order->status === 'assigned' && !$order->driver_id): ?>
                                <?php if($order->isPaid()): ?>
                                    <button data-order-id="<?php echo e($order->id); ?>" 
                    ...
                    <!-- Price Breakdown Row -->
                    <tr id="breakdownRow<?php echo e($order->id); ?>" style="display:none;">
                        <td colspan="10" style="padding:4px 8px 12px 8px;">
                            <div class="price-breakdown-box show" id="breakdownBox<?php echo e($order->id); ?>">
                                <div style="display:grid; grid-template-columns:1fr 1fr 1fr 1fr; gap:8px; font-size:12px;">
                                    <div>
                                        <div class="row"><span class="label">Base Fare</span><span class="value"><?php echo e(number_format($order->base_fare ?? 0, 0, ',', ' ')); ?> F</span></div>
                                        <div class="row"><span class="label">Distance (<?php echo e($order->distance_km ?? 0); ?> km)</span><span class="value"><?php echo e(number_format($order->distance_charge ?? 0, 0, ',', ' ')); ?> F</span></div>
                                        <div class="row"><span class="label">Weight (<?php echo e($order->weight_kg ?? 0); ?> kg)</span><span class="value"><?php echo e(number_format($order->weight_charge ?? 0, 0, ',', ' ')); ?> F</span></div>
                                    </div>
                                    <div>
                                        <div class="row"><span class="label">Subtotal</span><span class="value" style="font-weight:600;"><?php echo e(number_format(($order->base_fare ?? 0) + ($order->distance_charge ?? 0) + ($order->weight_charge ?? 0), 0, ',', ' ')); ?> F</span></div>
                                        <div class="row"><span class="label">Service Fee (5%)</span><span class="value"><?php echo e(number_format($order->service_fee ?? 0, 0, ',', ' ')); ?> F</span></div>
                                        <div class="row"><span class="label">VAT (<?php echo e($order->tax_rate ?? 5); ?>%)</span><span class="value"><?php echo e(number_format($order->tax_amount ?? 0, 0, ',', ' ')); ?> F</span></div>
                                    </div>
                                    <div>
                                    ...
                                    <div style="background:#F1F5F9; border-radius:8px; padding:8px 10px;">
                                        ...
                                        <div style="font-size:11px; color:#64748B;">
                                            <div>Distance: <strong><?php echo e($order->distance_km ?? 0); ?> km</strong></div>
                                            <div>Weight: <strong><?php echo e($order->weight_kg ?? 0); ?> kg</strong></div>
                                            <div>Customer: <strong><?php echo e($order->customer->name ?? 'N/A'); ?></strong></div>
                                            <?php if($order->status === 'delivered'): ?>
                                                <div style="color:#10B981;">✅ Delivered: <?php echo e(optional($order->actual_delivery)->format('M d, Y h:i A') ?? 'N/A'); ?></div>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </div>
...
<?php echo $__env->make('layouts.dashboard', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\logistics_system\resources\views/admin/orders.blade.php ENDPATH**/ ?>